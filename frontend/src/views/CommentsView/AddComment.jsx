...
const AddComment = () => {
  ...
  const onSubmit = (data) => {
    ...
    if (title.length <= 1)
      return alert("Title must be longer than one character!");
    if (content.length <= 3)
      return alert("Content must be at least 3 characters long!");
    ...
    axios
      .post(`http://localhost:8081/games/game/${id}/comments`, payload)
      .then((res) => {
