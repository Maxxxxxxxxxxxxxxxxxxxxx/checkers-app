...
const randomId = () => [...Array(5)].map(e => Math.floor(Math.random() * 9))
const positionsTop = [
    [1, 0],[3, 0],[5, 0],[7, 0],
    [0, 1],[2, 1],[4, 1],[6, 1],
    [1, 2],[3, 2],[5, 2],[7, 2] 
...
const mapPawns = (side, positions) => positions.map((field, index) => {
    return {
        isQueen: false,
        isDead: false,
        side,
...
class GameState {
    constructor(colorTop, colorBottom) {
        ...
        this.id = randomId()
        this.moves = []
        this.pawns = [mapPawns(colorTop, positionsTop), mapPawns(colorBottom, positionsBottom)].flatMap(e => e)
    }
    // move = (side, index, x, y) => {
    //     this.pawns = this.pawns.map(pawn => pawn.pos.x === destX && pawn.pos.y === destY ? { ...pawn, pos: { x: destX, y: destY } } : pawn);
    // }
    validateMove = (pawn, destX, destY) => {
        ...
        if (!pawn.isQueen) {
            ...
            if (xShift === 1 && yShift === 1) {
                ...
                if (this.pawns.find(pawn => pawn.pos.x === destX && pawn.pos.y === destY)) return false
                else this.pawns = this.pawns.map(pawn => pawn.pos.x === destX && pawn.pos.y === destY ? { ...pawn, pos: { x: destX, y: destY } } : pawn);
                return true
            }
            if (xShift === 2 && yShift === 2) {
                ...
                if (this.pawns.find(p => p.pos.x === enemyVec.x && p.pos.y === enemyVec.y && p.side != pawn.side)) {
                    // kill the enemy pawn
